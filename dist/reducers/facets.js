...
function setGlobalFilter(state, action) {
    ...
    var globalFilters = updateObjectAtKey(state.globalFilters, filter, key);
    return updateObject(state, { globalFilters: globalFilters });
}
        ...
        switch (facet.type) {
            case "CheckboxFacet":
                ...
                Object.keys(facet.values).forEach(function (value) {
                    ...
                    var item = updateObject(currentItem, { selected: false, count: 0 });
                    values_1[value] = item;
                });
                facets[key] = updateObject(facet, { values: values_1, filterClause: "" });
                break;
            case "RangeFacet":
...
function setCheckboxFacetValues(facet, facetResults) {
    ...
    return updateObject(facet, { values: values, filterClause: "" });
}
function updateFacetsValues(state, action) {
    ...
    var facets = updateObject(state.facets, updatedFacets);
    return updateObject(state, { facets: facets });
}
function mergeCheckboxFacetValues(facet, facetResults) {
    ...
    return updateObject(facet, { values: values });
}
function setFacetMode(state, action) {
    var facetMode = action.facetMode;
    return updateObject(state, { facetMode: facetMode });
}
...
function addCheckboxFacet(state, action) {
    ...
    var facets = updateObjectAtKey(state.facets, checkFacet, key);
    return updateObject(state, { facets: facets });
}
function toggleFacetSelection(state, action) {
    ...
    var updatedFacetItem = updateObject(oldFacetItem, { selected: !oldFacetItem.selected });
    var newValue = {};
    var values = updateObjectAtKey(checkboxFacet.values, updatedFacetItem, value.toString());
    var newFacet = updateObject(checkboxFacet, { values: values });
    var filterClause = buildCheckboxFilter(newFacet);
    var newFacetWithFilter = updateObject(newFacet, { filterClause: filterClause });
    var facets = updateObjectAtKey(state.facets, newFacetWithFilter, key);
    return updateObject(state, { facets: facets });
}
function setFacetRange(state, action) {
    ...
    var newRangeFacet = updateObject(existingRangeFacet, { filterLowerBound: lowerBound, filterUpperBound: upperBound });
    ...
    var newFacetWithFilter = updateObject(newRangeFacet, { filterClause: filter, facetClause: facetClause });
    var facets = updateObjectAtKey(state.facets, newFacetWithFilter, key);
    return updateObject(state, { facets: facets });
}
